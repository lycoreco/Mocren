...
test_sites: list[TestSite] = [
    ...
    },
    {
        ...
        'url': f'https://jikkyo.tsukumijima.net/api/kakolog/jk1?starttime={int(time.time() - 7200)}&endtime={int(time.time() - 3600)}&format=xml',
        'normal_status_code': 200,
        'normal_response_data': '<packet>\n<chat thread="',
    },
    {
        'name': 'ニコニコ実況 過去ログ API - JSON API (Latest jk1)',
        'url': f'https://jikkyo.tsukumijima.net/api/kakolog/jk1?starttime={int(time.time() - 7200)}&endtime={int(time.time() - 3600)}&format=json',
        'normal_status_code': 200,
        'normal_response_data': '{"packet":[{"chat":{"thread":"',
    },
    {
        'name': 'ニコニコ実況 過去ログ API - XML API (Latest jk141)',
        'url': f'https://jikkyo.tsukumijima.net/api/kakolog/jk141?starttime={int(time.time() - 7200)}&endtime={int(time.time() - 3600)}&format=xml',
        'normal_status_code': 200,
        'normal_response_data': '<packet>\n<chat thread="',
    },
    {
        'name': 'ニコニコ実況 過去ログ API - JSON API (Latest jk141)',
        'url': f'https://jikkyo.tsukumijima.net/api/kakolog/jk141?starttime={int(time.time() - 7200)}&endtime={int(time.time() - 3600)}&format=json',
        'normal_status_code': 200,
        'normal_response_data': '{"packet":[{"chat":{"thread":"',
    },
    {
